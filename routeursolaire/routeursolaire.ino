/*
  ROUTEURSOLAIRE_ESP.ino - ESP8266 program that provides a web interface and a API for EcoPV 3+
...
#include "routeursolaire_options.h"
...
#if defined (ROUTEURSOLAIRE_FTP_SERVER)
...
// Importation des définitions de configuration générale de ROUTEURSOLAIRE!
#include "routeursolaire_defines.h"
...
// ***********************************************************************************
// ******************* Variables globales de configuration ROUTEURSOLAIRE! ********************
...
// Définition des paramètres RelayPlus
int relayPlusMin    = DEFAULT_RELAYPLUS_MIN;         // Temps minimum de fonctionnement journalier
...
// Flag indiquant la nécessité de rebooter ROUTEURSOLAIRE!
bool shouldReboot = false;
// Flag indiquant la nécessité de sauvegarder la configuration de ROUTEURSOLAIRE!
...
// Variables pour la gestion du mode relayPlus
long relaisPlusIndex = -1;      // Index du temps de fontionnement du relais comme référence journalière
                                // -1 : non initialisé
int relaisPlusCountDown = -1;   // compte à rebours de la durée d'une journée moins 10 minutes
...
//Initialisation des variables globales de type String
  ...
  msg = F("Version : ");
  msg += ROUTEURSOLAIRE_VERSION_FULL;
  logSerial ( F("[ESP]"), msg ); 
  ...
  // On teste l'existence du fichier de configuration de ROUTEURSOLAIRE!
  ...
  // Sauvegarde de la configuration ROUTEURSOLAIRE! si nécessaire
  if (shouldSaveConfig) {
    configWrite();
...
*/
  ...
  // Démarrage mDNS
  if (MDNS.begin(ROUTEURSOLAIRE_MDNS)) {
    delay (10);
...
bool configRead(const String& configString)
{
  if (configString != "")
  {
    // buffer pour manipuler le fichier de configuration de ROUTEURSOLAIRE!
    ...
    if (!error)
    {
      if (jsonConfig.containsKey("ip"))   // Il faut au moins que l'IP soit présente pour que le fichier soit considéré comme valide
      {
        ...
        if (jsonConfig.containsKey("RelayPlus_min"))      relayPlusMin = jsonConfig["RelayPlus_min"]; else shouldSaveConfig = true;
        ...
        if ( relayPlusMax <= relayPlusMin)  relayPlusMax = relayPlusMin + 5;
      }
      else return false;
    }
    else return false;
...
void configWrite(void)
{
  ...
  jsonConfig["RelayPlus_min"] = relayPlusMin;
...
void setParamEcoPV(const String& param, const String& value)
{
  ...
  // pour mettre à jour dans ROUTEURSOLAIRE
  // ce qui permet de vérifier la prise en compte de la commande
  // C'est par ce seul moyen de ROUTEURSOLAIRE met à jour sa base interne des paramètres
}
...
///////////////////////////////////////////////////////////////////
// Active ou arrête le mode Relay+ dans EcoPV                    //
// qui est en fait un Timer sur le relais                        //
///////////////////////////////////////////////////////////////////

void relayPlusEcoPV(long bT)
{
  String command;
  command.reserve(32);
...
{  
  ...
  // Toutes les REMOTE_RELAY_MIRRORING_PERIOD secondes, si activé, on recopie l'état du relais physique ROUTEURSOLAIRE vers le relais miroir
  ...
  // Traitement RelaisPlus toutes les minutes
...
void onMqttConnect(bool sessionPresent) 
{
  ...
  String deviceID = F("ROUTEURSOLAIRE");
  ...
  configPayloadTemplate.replace(F("#VERSION#"), F(ROUTEURSOLAIRE_VERSION));
...
/*
    ...
    mqttClient.publish("ROUTEURSOLAIRE/relaisPlusIndex", 0, true, String(relaisPlusIndex).c_str());
    yield();
    mqttClient.publish("ROUTEURSOLAIRE/relayPlusMin", 0, true, String(relayPlusMin).c_str());
    yield();
    mqttClient.publish("ROUTEURSOLAIRE/relayPlusMax", 0, true, String(relayPlusMax).c_str());
    yield();
    mqttClient.publish("ROUTEURSOLAIRE/relayPlusCountDown", 0, true, String(relaisPlusCountDown).c_str());
    yield();
    long condition = ( relayPlusMin - ( long (ecoPVStats[INDEX_RELAY].toInt()) - relaisPlusIndex ) );
    mqttClient.publish("ROUTEURSOLAIRE/relaisPlusCondition", 0, true, String(condition).c_str());
    yield();
*/
...
void startWeb (void) {
  AsyncElegantOTA.setID(ROUTEURSOLAIRE_VERSION_FULL);
...
void setWebHandlers (void) {
  ...
  webServer.on("/ROUTEURSOLAIRE.css", HTTP_ANY, [](AsyncWebServerRequest * request)
  {
    request->send(LittleFS, F("/ROUTEURSOLAIRE.css"), "text/css");
  });
  ...
  // Download le fichier de configuration de ROUTEURSOLAIRE!
  ...
  {
    String response = "";
    if ( request->hasParam ( F("configROUTEURSOLAIRE") ) )
      ...
      else if ( request->hasParam ( F("versionROUTEURSOLAIRE") ) )
        response = F(ROUTEURSOLAIRE_VERSION);
      ...
      else if ( request->hasParam ( F("relayplusstate") ) ) {
        if (relaisPlusIndex == -1) response = F("OFF");
          else response = F("ON");
      }
  ...
  webServer.on("/api/set", HTTP_GET, [](AsyncWebServerRequest * request)
  {
    ...
    if ( ( request->hasParam ( F("param") ) ) && ( request->hasParam ( F("value") ) )
      ...
      // Note : la mise à jour de la base interne de ROUTEURSOLAIRE se fera de manière asynchrone
      // setParamEcoPV() demande à EcoPV de renvoyer tous les paramètres à ROUTEURSOLAIRE
    }
    ...
    else if ( ( request->hasParam ( F("configROUTEURSOLAIRE") ) ) && ( request->hasParam ( F("value") ) ) ) {
      mystring = request->getParam(F("value"))->value();
